...
void drawLayout(){
  ...
  textAlign(CENTER);
  fill(0);
  text("Exit (Rechtsklick)",SCREENX/2,23);
  textAlign(LEFT);
...
if(onMenuScreen==true){
if(selection1 == true && mouseButton == LEFT){
  score =0 ;
  onMenuScreen = false;
  gameSetup = true;
}
else if(selection2 == true && mouseButton == LEFT){
  onMenuScreen = false;
  onHowToPlayScreen = true;
}
else if(selection3 == true && mouseButton == LEFT){
  onMenuScreen = false;
 ...
 if(mouseButton == LEFT && bulletCooldown == 0){
